...
export default function DashboardPage() {
  ...
  const { user, token, initialized, logout, initialize } = useAuthStore();
  ...
  // Initialize socket for presenter slide synchronization (e.g. multi-tab subscreen sync)
  // Join room named 'LUNA_PRESENTATION_ROOM'
  ...
  if (!initialized) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center flex-col gap-3">
        ...
        <p className="text-sm text-slate-400">인증 세션 확인 중...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col h-screen overflow-hidden">
      ...
      <header className="border-b border-slate-900 bg-slate-900/50 backdrop-blur-md px-6 py-4 flex items-center justify-between z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-extrabold text-base leading-tight">LUNA Presenter</h1>
            <p className="text-[10px] text-slate-500">
              관리자 계정: <span className="text-indigo-400 font-semibold">{user.username}</span>
            </p>
          </div>
        </div>
        ...
        <button
          onClick={logout}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-800 bg-slate-900/40 hover:bg-rose-950/20 hover:border-rose-500/30 hover:text-rose-400 transition-all duration-200 text-xs"
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2.5 custom-scrollbar">
            ...
            {isLoading ? (
              ...
              <div className="py-12 text-center text-xs text-slate-600">
                개설된 강의 목차가 없습니다. [+ 새 강의] 버튼을 눌러보세요.
              </div>
            ) : (
