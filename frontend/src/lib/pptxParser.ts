...
export interface SlideParsedData {
  title: string;
  subtitle: string;
  content: string;
  gradient: string;
 ...
 * @returns {SlideParsedData[]} List of slide card objects containing title, subtitle, content, and gradient.
 */
export function parsePptx(filePath: string): SlideParsedData[] {
  try {
    ...
    const parsedSlides: SlideParsedData[] = [];
    const gradients = [
